...
if(isset($_SESSION['id']))
{
    ...
    $reservations = $dbh->query("SELECT * FROM bretagne_nord_reservation"); //Récupère toutes les infos pour afficher les reservations bretagne nord
        ...
        if(!empty($_POST['date']) AND !empty($_POST['duree']) AND !empty($_POST['nb_personne']) AND !empty($_POST['niveau']) AND !empty($_POST['nom']) AND !empty($_POST['mailres']) AND !empty($_POST['num_res']))
        {
            ...
            if($dureelenght <= 100)
            {
                if($nb_personne <= 10)
                {
                    ...
                    $i=0;
                    while ($i!=$duree) {
                      $insertbretagne_nord_reservation = $dbh->prepare("INSERT INTO `bretagne_nord_reservation` (`id`, `date`, `duree`, `nb_personne`, `niveau`, `nom`, `mail`, `num_res`, `verif_unique`) VALUES (NULL, ?, ?, ?, ?, ?, ?, ?, ?);");
    ...
    if(isset($_POST['form_delete_reservation']))
    {
      ...
      $deletebretagne_nord_reservation = $dbh->prepare('DELETE FROM `bretagne_nord_reservation` WHERE `num_res` = ?');
