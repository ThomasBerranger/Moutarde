...
if(isset($_POST['formconnexion']))
{
    ...
    {
        $requser = $dbh->prepare("SELECT * FROM membres WHERE mail= ? AND  mdp = ?");
...
<![endif]-->
    ...
    <body id="page-top" class="index w3-animate-opacity">
        ...
        <?php
        $mail = $dbh->prepare("SELECT mail FROM membres WHERE id = ?");
        $mail->execute(array($_SESSION['id']));
        $mail = $mail->fetch();

        $reservation_bretagne_nord = $dbh->query("SELECT * FROM bretagne_nord_reservation");
        ?>
